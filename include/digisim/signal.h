...
/**
 * Get a pointer to uint64_t representing the HIGH/LOW value of a signal.
 *
 * There are DI_SIGNAL_U64_COUNT(signal->bits) uint64_ts in this array.
 *
 * After the values array, the error array (di_signal_get_error) starts at the next u64.
 * After the error array, the unknown array (di_signal_get_unknown) starts at the next u64.
 *
 * All 3 arrays are contiguous, for a total of 3 * DI_SIGNAL_U64_COUNT(signal->bits) u64s.
 *
 * @memberof DiSignal
 * @param signal The signal to inspect.
 * @return A mutable list of u64s, where each bit represents a HIGH/LOW value of a wire.
 */
uint64_t *di_signal_get_values(DiSignal *signal);

/**
 * Get a pointer to uint64_t representing whether the wire carries an error signal.
 * Errors can be generated by connecting a high and low signal to the same wire.
 *
 * There are DI_SIGNAL_U64_COUNT(signal->bits) uint64_ts in this array.
 *
 * @memberof DiSignal
 * @param signal The signal to inspect.
 * @return A mutable list of u64s, where a 1 bit means this wire carries an error signal.
 */
uint64_t *di_signal_get_error(DiSignal *signal);

/**
 * Get a pointer to uint64_t representing whether the wire carries an unknown signal (floating).
 *
 * @memberof DiSignal
 * @param signal The signal to inspect.
 * @return A mutable list of u64s, where a 1 bit means this wire is not connected to HIGH or LOW.
 */
